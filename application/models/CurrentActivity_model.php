...
class CurrentActivity_model extends CI_Model {
    ...
    function getAutoCompleteData($searchStr) {
        $query = "select id,activity FROM $this->table where activity like '%" . $searchStr . "%' and status = 1";
        $exeQue = $this->db->query($query);
        return $resulArr = $exeQue->result_array();
    }

    function getRecordId($record) {
        $this->db->select("id,activity");
        $this->db->from($this->table);
        $this->db->where('activity', $record);
