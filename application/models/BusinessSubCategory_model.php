...
class BusinessSubCategory_model extends CI_Model {

    function __construct() {

        parent::__construct();

        $this->table = "business_sub_categories";
    }

    function get_datatables($searchWhere = "", $searchParamVal = "", $start = "", $length = "", $orderBy = "", $orderByVal = "") {

        //define columns for searching
        $aColumns = array("business_sub_categories.id", "business_sub_categories.name", "business_categories.name");

        // $this->db->select('tu.*, tm.*, tc.countryName, tc.countryCode,tu.createdDate as registeredDate');
        $this->db->select("business_sub_categories.id,business_sub_categories.name,business_categories.name as business_category,IF(business_sub_categories.updated_on != '', business_sub_categories.updated_on, business_sub_categories.created_on) as updated");
        $this->db->from($this->table);
        $this->db->join('business_categories', 'business_categories.id = ' . $this->table . '.business_category_id', 'left');
        $this->db->where('business_sub_categories.status','1');
    ...
    function getRecordId($record, $business_category_id) {
    ...
    function getBusinessSubCategory($business_category_id) {
        $this->db->select("id,name");
        $this->db->from($this->table);
        $this->db->where('business_category_id', $business_category_id);
