...
class LocalCommunity extends Custom_Controller {

    public function __construct() {

        parent::__construct();

        $this->load->model('LocalCommunity_model');
        $this->load->model('SubCommunity_model');

        $this->controllerName = "localCommunity";
        $this->singular = "Local Community";
        $this->plural = "Local Community";
    }
    ...
    public function index($id = "") {
        ...
        if ($id == "") {
            ...
            $data['item'] = $this->LocalCommunity_model->get_record($id);
        }
    ...
    public function data_list() {
        ...
        $dataList = $this->LocalCommunity_model->get_datatables("", $sWhere, $start, $length, "local_community.id", "DESC");
        $dataListCount = $this->LocalCommunity_model->get_datatables("", $sWhere);
    ...
    public function add() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->LocalCommunity_model->add_record($data);
    ...
    public function edit($id) {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->LocalCommunity_model->update_record($id, $data);
    ...
    public function delete($id) {
        ...
        $this->LocalCommunity_model->delete_record($id);
    ...
    public function deleteMultiple() {
        if (isset($_POST['id'])) {
            $ids = $_POST['id'];
            $this->LocalCommunity_model->delete_multiple($ids);
            $response = array(
                'errorcode' => 1,
                'action' => 'MUL_DELETE'
            );
