...
class CurrentActivity extends Custom_Controller {

    public function __construct() {

        parent::__construct();

        $this->load->model('CurrentActivity_model');

        $this->controllerName = "currentActivity";
        $this->singular = "Current Activity";
        $this->plural = "Current Activity";
    }
    ...
    public function index($id = "") {
        ...
        if ($id == "") {
            ...
            $data['item'] = $this->CurrentActivity_model->get_record($id);
        }
    ...
    public function data_list() {
        ...
        $dataList = $this->CurrentActivity_model->get_datatables("", $sWhere, $start, $length, "id", "DESC");
        $dataListCount = $this->CurrentActivity_model->get_datatables("", $sWhere);
    ...
    public function add() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->CurrentActivity_model->add_record($data);
    ...
    public function edit($id) {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->CurrentActivity_model->update_record($id, $data);
    ...
    public function delete($id) {
        ...
        $this->CurrentActivity_model->delete_record($id);
    ...
    public function deleteMultiple() {
        if (isset($_POST['id'])) {
            $ids = $_POST['id'];
            $this->CurrentActivity_model->delete_multiple($ids);
            $response = array(
                'errorcode' => 1,
                'action' => 'MUL_DELETE'
            );
