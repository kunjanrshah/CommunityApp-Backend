...
class SubCasts extends Custom_Controller {

    public function __construct() {

        parent::__construct();

        $this->load->model('Subcast_model');

        $this->controllerName = "subCasts";
        $this->singular = "Sub Cast";
        $this->plural = "Sub Casts";
    }
    ...
    public function index($id = "") {
        ...
        if ($id == "") {
            ...
            $data['item'] = $this->Subcast_model->get_record($id);
        }
    ...
    public function data_list() {
        ...
        $dataList = $this->Subcast_model->get_datatables("", $sWhere, $start, $length, "id", "DESC");
        $dataListCount = $this->Subcast_model->get_datatables("", $sWhere);
    ...
    public function add() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->Subcast_model->add_record($data);
    ...
    public function edit($id) {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->Subcast_model->update_record($id, $data);
    ...
    public function delete($id) {
        ...
        $this->Subcast_model->delete_record($id);
    ...
    public function deleteMultiple() {
        if (isset($_POST['id'])) {
            $ids = $_POST['id'];
            $this->Subcast_model->delete_multiple($ids);
            $response = array(
                'errorcode' => 1,
                'action' => 'MUL_DELETE'
            );
