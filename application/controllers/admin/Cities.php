...
class Cities extends Custom_Controller {
    ...
    public function index($id = "") {
        ...
        if ($id == "") {
            ...
            $data['item'] = $this->City_model->get_record($id);
        }
    ...
    public function data_list() {
        ...
        $dataList = $this->City_model->get_datatables("", $sWhere, $start, $length, "cities.id", "DESC");
        $dataListCount = $this->City_model->get_datatables("", $sWhere);
    ...
    public function add() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->City_model->add_record($data);
    ...
    public function edit($id) {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $data['updated_at'] = date('Y-m-d H:i:s');
            $menu = $this->City_model->update_record($id, $data);
    ...
    public function delete($id) {
        ...
        $this->City_model->delete_record($id);
    ...
    public function deleteMultiple() {
        if (isset($_POST['id'])) {
            $ids = $_POST['id'];
            $this->City_model->delete_multiple($ids);
            $response = array(
                'errorcode' => 1,
                'action' => 'MUL_DELETE'
            );
