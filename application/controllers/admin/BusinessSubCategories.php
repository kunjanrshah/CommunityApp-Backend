...
class BusinessSubCategories extends Custom_Controller {

    public function __construct() {

        parent::__construct();

        $this->load->model('BusinessSubCategory_model');
        $this->load->model('BusinessCategory_model');

        $this->controllerName = "businessSubCategories";
        $this->singular = "Business Sub Category";
        $this->plural = "Business Sub Categories";
    }
    ...
    public function index($id = "") {
        ...
        if ($id == "") {
            ...
            $data['item'] = $this->BusinessSubCategory_model->get_record($id);
        }
    ...
    public function data_list() {
        ...
        $dataList = $this->BusinessSubCategory_model->get_datatables("", $sWhere, $start, $length, "business_sub_categories.id", "DESC");
        $dataListCount = $this->BusinessSubCategory_model->get_datatables("", $sWhere);
    ...
    public function add() {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->BusinessSubCategory_model->add_record($data);
    ...
    public function edit($id) {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            $menu = $this->BusinessSubCategory_model->update_record($id, $data);
    ...
    public function delete($id) {
        ...
        $this->BusinessSubCategory_model->delete_record($id);
    ...
    public function deleteMultiple() {
        if (isset($_POST['id'])) {
            $ids = $_POST['id'];
            $this->BusinessSubCategory_model->delete_multiple($ids);
            $response = array(
                'errorcode' => 1,
                'action' => 'MUL_DELETE'
            );
